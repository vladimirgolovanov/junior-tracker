...
SCENARIO_1 = (
    {
        ...
        "events": [
            {"type_name": "sleep_end",   "occurred_at": datetime(2026, 1, 16, 7, 30, tzinfo=timezone.utc)},
            {"type_name": "sleep_start", "occurred_at": datetime(2026, 1, 16, 10, 0, tzinfo=timezone.utc)},
            {"type_name": "sleep_end",   "occurred_at": datetime(2026, 1, 16, 11, 0, tzinfo=timezone.utc)},
            {"type_name": "sleep_start", "occurred_at": datetime(2026, 1, 16, 19, 0, tzinfo=timezone.utc)},
        ],
    },
    date(2026, 1, 17),
    {
...
SCENARIO_2 = (
    {
        ...
        "events": [
            {"type_name": "sleep_end",   "occurred_at": datetime(2026, 1, 19, 7, 0,  tzinfo=timezone.utc)},
            {"type_name": "sleep_start", "occurred_at": datetime(2026, 1, 19, 9, 30, tzinfo=timezone.utc)},
            {"type_name": "sleep_end",   "occurred_at": datetime(2026, 1, 19, 11, 0, tzinfo=timezone.utc)},
            {"type_name": "sleep_start", "occurred_at": datetime(2026, 1, 19, 13, 0, tzinfo=timezone.utc)},
            {"type_name": "sleep_end",   "occurred_at": datetime(2026, 1, 19, 14, 30, tzinfo=timezone.utc)},
            {"type_name": "sleep_start", "occurred_at": datetime(2026, 1, 19, 19, 30, tzinfo=timezone.utc)},
        ],
    },
    date(2026, 1, 20),
    {
...
@pytest.mark.parametrize("seed, today, expected", [SCENARIO_1, SCENARIO_2])
@pytest.mark.asyncio
async def test_dashboard_get_last_three_days(session, seed, today, expected):
    ...
    for key, val in expected.items():
        assert yesterday[key] == val, f"Mismatch on {key!r}: {yesterday[key]!r} != {val!r}"
